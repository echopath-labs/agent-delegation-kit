...
import { readTaskState, removeTaskIntegrityAnchor, transitionTaskStateMatching } from "./state.mjs";
...
export async function recordTerminalDecision(prepared, review, action, actor, options = {}) {
  ...
  let authoritativeReview = review;
  if (action === "accept") {
    authoritativeReview = await buildHostReviewPacket(prepared, {
    ...
    if (!sameReviewBasis(review, authoritativeReview)) {
      throw new DelegationError("stale_review", "Candidate or host evidence changed after review; rebuild the review before acceptance.");
    }
    if (authoritativeReview.packet.acceptance.eligible !== true) {
