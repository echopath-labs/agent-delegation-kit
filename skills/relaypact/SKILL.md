...
1. Resolve the Skill-local wrapper and run `support` plus `doctor` without
   ...
   reading authentication. Confirm the target Git root and read the nearest
...
6. Select a host-approved named Codex worker profile and default external Codex
...
11. Issue an explicit terminal decision only within user-granted authority.
