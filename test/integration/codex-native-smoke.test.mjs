...
const enabled = process.env.RELAYPACT_NATIVE_CODEX_SMOKE === "1";
...
test("opt-in native Codex delegated execution", { skip: !enabled }, async (context) => {
  ...
  if (process.env.RELAYPACT_NATIVE_CODEX_PROFILE) profile.codexProfile = process.env.RELAYPACT_NATIVE_CODEX_PROFILE;
  if (process.env.RELAYPACT_NATIVE_CODEX_MODEL) profile.model = process.env.RELAYPACT_NATIVE_CODEX_MODEL;
  if (process.env.RELAYPACT_NATIVE_CODEX_REASONING) profile.reasoning = process.env.RELAYPACT_NATIVE_CODEX_REASONING;
  const scenario = {};
